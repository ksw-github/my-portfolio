...
export const projects: Project[] = [
    ...
    features: [
      ...
      },
      {
        ...
        items: [
          ...
          "커스텀 훅으로 리팩토링하여, 3개 컴포넌트에 산재된 우클릭 로직을 1개 훅으로 통합",
          "Zustand기반 장비 상태 관리 스토어 구축, 기존 팝업 스토어에 AI 타입 관련 액션 확장",
        ],
      },
      {
        ...
        items: [
          ...
          "마우스스크롤 커스텀 훅 개발, 장비 목록/영상 목록 2개 패널에서 마우스 드래그 스크롤 기능 공통 적용, 리사이즈 대응 포함",
          "SCSS 리팩토링: 장비관리와 하단 영상스트리밍 모듈의 스타일 코드 네스팅 구조 정리",
        ],
      },
      {
        ...
        items: [
          ...
          "CCTV/로봇 아이콘에 CSS 애니메이션 이펙트 적용, 장비 상태 변경 시 시각적 피드백 제공",
      ...
      },
      {
        ...
        items: [
          ...
          "영상 스트리밍 패널을 포함한 관제 전체 영역 구축",
          "팝업 컴포넌트 구현으로 팝업 관련 중복 SCSS 코드 대폭 축소, 로그아웃 팝업 코드량 66% 축소",
          "`useContextMenu` 훅 추출로 우클릭 메뉴 로직의 유지보수 포인트 3곳 → 1곳 통합",
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        title: "성과",
        items: [
          "계정관리 3개 모듈(관리자/사용자/임시사용자) 전체 CRUD를 구현하여 계정 관리 기능 100% 완성",
          "리팩토링을 통해 중복 모달 코드 40% 감소 및 에러 핸들링 구체화로 운영 이슈 추적 시간 약 50% 단축",
          "공통 컴포넌트(CommonSearch, CommonTable, CommonModal) 재사용 패턴 적용으로 페이지 신규 개발 시 평균 개발 시간 약 30% 단축",
          "폼 유효성 검증(아이디/이메일 중복검사, 비밀번호 복합 조건, 필수값 체크)을 프론트단에서 100% 처리하여 불필요한 서버 요청 방지",
        ],
      },
    ],
  },
  {
    ...
    desc: "공사 작업자, 업체, 관리자가 모바일 웹/PC 웹에서 공사허가 및 출입 신청, 완료보고, 공사중지, 안전교육, 안전사항 등을 확인하고 본인 서명을 전자문서화하여 관리하는 시스템으로 작업자/업체용 화면(모바일 웹)과 관리자용 화면(PC 웹)을 담당하였습니다.",
